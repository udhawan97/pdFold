...
PDFKit owns display and composition (`PDFView`, `PDFDocument`, `PDFPage`, selection, search, print). PDFium is a secondary engine for image-level work, import validation, and glyph geometry.

<Details summary="Why two rendering engines instead of one">
PDFKit is the native, best-supported path for showing and composing pages. PDFium is reached only where PDFKit is the wrong tool: downsampling embedded images during compression, a non-blocking validation smoke check on import, and reporting the actual affine transforms of text glyphs for in-place editing. PDFium never blocks an import PDFKit is willing to open.
</Details>
