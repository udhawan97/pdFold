...
- **One page-object pass per canonical revision:** `PDFPageObjectInspection` owns the bounded PDFium enumeration and derives render-mode regions, `PageGraphicsIndex`, and `PageObjectMap`. The workspace retains every admitted PageRef under a precomputed member revision token until canonical bytes are explicitly invalidated—no fixed-size LRU and no whole-member hash on each lookup. Per-revision and process-wide page/projection-cost ceilings fail closed rather than evicting and rescanning accepted snapshots. Object selection projects committed absolute ops, live rotation, and permission state over the canonical map without rescanning.
- **One edit replay path:** `WorkspaceEditReplayEngine` materializes object operations and text operations from the same canonical member bytes. Structural edits remain on the PDFium `GenerateContent` → `SaveAsCopy` path; transparent text-only PDFs are imported as Form XObjects instead of redrawing the destination page; qpdf grafts live annotations/AcroForm state; and PDFium performs the final rotation/stamp `SaveAsCopy`. `reconcileCommittedEditsWithLoadedPages()` is member-atomic and replays both lanes after divergence. Pristine bytes persist for members edited through either lane.
